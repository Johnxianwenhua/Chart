...
- (void)layoutSubviews
{
    ...
    CGRect arrowRect = CGRectMake(0, (self.frame.size.height - YUFoldingIconWidth)/2 + 5, YUFoldingIconWidth - 10, YUFoldingIconWidth - 10);
    ...
    if (_arrowPosition == YUFoldingSectionHeaderArrowPositionRight) {
        arrowRect.origin.x = YUFoldingMargin * 2 + labelWidth * 2 - 15;
        titleRect.origin.x = YUFoldingMargin + 5;
...
- (void)shouldExpand:(BOOL)shouldExpand
{
    ...
    [UIView animateWithDuration:0.2  animations:^{
        if (shouldExpand) {
            if (weakself.arrowPosition == YUFoldingSectionHeaderArrowPositionRight) {
                weakself.arrowImageView.transform = CGAffineTransformMakeRotation(-M_PI/2);
            }else{
                weakself.arrowImageView.transform = CGAffineTransformMakeRotation(M_PI/2);
            }
        } else {
        ...
        if (weakself.autoHiddenSeperatorLine) {
            if (finished == YES) {
                weakself.separatorLine.hidden = shouldExpand;
            }
        }
    }];
}
